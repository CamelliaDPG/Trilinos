...
/** \file   Intrepid2_OrientationToolsDefPermutations.hpp
    \brief  Definition file for the Intrepid2::OrientationTools class.
    \author Created by Nate Roberts
*/
#ifndef __INTREPID2_ORIENTATIONTOOLS_DEF_PERMUTATIONS_HPP__
#define __INTREPID2_ORIENTATIONTOOLS_DEF_PERMUTATIONS_HPP__
...
template<typename OrientationViewType, typename BasisType>
inline
bool
  OrientationTools<DT>::orientationsArePermutations(const OrientationViewType orts,
                                                    const BasisType * basis)
  {
    ...
    if ((cellDim < 3) || basis->requireOrientation()) {
      ...
      const ordinal_type numCells  = orts.extent(0);
      if (basis->requireOrientation()) {
        numEdges = cellTopo.getEdgeCount()*ordinal_type(basis->hasDofsForSubcellDim(1));
        numFaces = cellTopo.getFaceCount()*ordinal_type(basis->hasDofsForSubcellDim(2));
      }
      
      using HostExecSpace = Kokkos::DefaultHostExecutionSpace;
      auto ortsHost = Kokkos::create_mirror_view_and_copy(typename HostExecSpace::memory_space(),orts);
      std::set<Orientation> ortSet;
      for (int i=0; i<ortsHost.extent_int(0); i++)
      {
        ortSet.insert(ortsHost(i));
      }
      
      std::vector<std::set<ordinal_type>> edgeOrientationSets(numEdges,std::set<ordinal_type>()); // one set per edge in the cell
      std::vector<std::set<ordinal_type>> faceOrientationSets(numFaces,std::set<ordinal_type>()); // one set per face in the cell
      
      for (const auto & ort : ortSet)
      {
        if (numEdges > 0)
        {
          std::vector<ordinal_type> edgeOrientations(numEdges);
          ort.getEdgeOrientation(&edgeOrientations[0], numEdges);
          for (int edge=0; edge<numEdges; edge++)
          {
            edgeOrientationSets[edge].insert(edgeOrientations[edge]);
          }
        }
        if (numFaces > 0)
        {
          std::vector<ordinal_type> faceOrientations(numFaces);
          ort.getFaceOrientation(&faceOrientations[0], numFaces);
          for (int face=0; face<numFaces; face++)
          {
            faceOrientationSets[face].insert(faceOrientations[face]);
          }
        }
      }
      int maxEdgeOrtCount = 0;
      int maxFaceOrtCount = 0;
      
      for (const auto & edgeOrientationSet : edgeOrientationSets)
      {
        maxEdgeOrtCount = max(static_cast<ordinal_type>(edgeOrientationSet.size()), maxEdgeOrtCount);
      }
      for (const auto & faceOrientationSet : faceOrientationSets)
      {
        maxFaceOrtCount = max(static_cast<ordinal_type>(faceOrientationSet.size()), maxFaceOrtCount);
      }
      
      using DeviceType   = typename OrientationViewType::device_type;
      using LayoutType   = typename OrientationViewType::array_layout;
      using MemoryTraits = typename OrientationViewType::memory_traits;
      ...
      // zero fill to account for ragged edges
      Kokkos::deep_copy(edgeOrientationsHost, 0);
      Kokkos::deep_copy(faceOrientationsHost, 0);
      
      for (int edge=0; edge<numEdges; edge++)
      {
        const auto &edgeOrientationSet = edgeOrientationSets[edge];
        const ordinal_type ortCount = static_cast<ordinal_type>(edgeOrientationSet.size());
        auto ortIt = edgeOrientationSet.begin();
        for (int i=0; i<ortCount; i++)
        {
          edgeOrientationsHost(edge,i) = *ortIt++;
        }
      }
      for (int face=0; face<numFaces; face++)
      {
        const auto &faceOrientationSet = faceOrientationSets[face];
        const ordinal_type ortCount = static_cast<ordinal_type>(faceOrientationSet.size());
        auto ortIt = faceOrientationSet.begin();
        for (int i=0; i<ortCount; i++)
        {
          faceOrientationsHost(face,i) = *ortIt++;
        }
      }
      Kokkos::deep_copy(faceOrientations,faceOrientationsHost);
      Kokkos::deep_copy(edgeOrientations,edgeOrientationsHost);
      
      int edgeDofsExist = (numEdges > 0) ? 1 : 0;
      const CoeffMatrixDataViewType matData = createCoeffMatrix(basis);
      
      int numNonPermutations = 0; // edges or faces that have a non-permutation.

      if (edgeDofsExist != 0)
      {
        Kokkos::parallel_reduce("Edge non-permutation counter", numEdges, KOKKOS_LAMBDA(const int edge, int& localCount)
        {
          ordinal_type ndofEdge = edgeDofCounts(edge);
      ...
      if ((numNonPermutations == 0) && (numFaces > 0))
      {
      ...
      return (numNonPermutations == 0);
    }
    else
    {
      return true; // "modify" methods leave things untouched; the identity operator is a permutation.
    }
  }
