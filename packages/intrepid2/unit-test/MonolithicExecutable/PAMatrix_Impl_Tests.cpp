  ...
  TEUCHOS_UNIT_TEST( PAMatrixImpl, GEMM )
{
  ...
  TEUCHOS_UNIT_TEST( PAMatrixImpl, GEMMPerformanceMatrixFree )
  {
    double relTol = 1e-13;
    double absTol = 1e-13;
...
TEUCHOS_UNIT_TEST( PAMatrixImpl, GEMMPerformanceFullMatrix )
{
  double relTol = 1e-13;
  double absTol = 1e-13;
  ...
  // Define column-major matrices A, B, C for each cell.
  // Compute C = A * B.
  // Here, we are emulating a full 3D contraction such as happens in assembled matrix apply
  // on an 8x8x8 hexahedral mesh, and estimating/measuring GEMM() throughput.
  // For polynomial order p, we construct a (p+1)^3 x (p+1)^3 matrix.  We then multiply against a (p+1)^3 vector representing solution coefficients.
  using DeviceType = DefaultTestDeviceType;
  using Scalar = double;
  
  const int cellCount = 512;
  
  std::vector<int> p_orders {1,2,4,6,8,10,12,20};
  
  for (auto p : p_orders)
  {
    const int M = (p+1)*(p+1)*(p+1); // row count for A, C
    const int N = 1; // new output dimension (column count for B, C)
    const int K = (p+1)*(p+1)*(p+1); // common dimension (A's column count, B's row count)
    ...
    using View1D = Kokkos::View<Scalar*,DeviceType>;
    View1D AView("PAMatrixImpl.GEMM FullMatrix test: AView", M * K);
    View1D BView("PAMatrixImpl.GEMM FullMatrix test: BView", cellCount * K * N);
    View1D CView("PAMatrixImpl.GEMM FullMatrix test: CView", cellCount * M * N);
    View1D CViewExpected("PAMatrixImpl.GEMM FullMatrix test: CViewExpected", cellCount * M * N);
